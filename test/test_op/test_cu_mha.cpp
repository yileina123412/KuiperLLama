...
TEST(test_mha_cu, mha_prefill_batch_cpu_vs_cuda) {
  ...
  // const int32_t kv_mul = 2;                                // GQA: kv_heads = head_num / kv_mul =
  // 2 const int32_t kv_dim = (head_num / kv_mul) * head_size;  // 16 const int32_t dim = head_num *
  // head_size;                // 32
  ...
  //     start_pos, head_num, layer_index, seq_len, kv_dim, kv_mul, head_size, seq_len, start_pos +
  //     T, ,out_cpu, q_cpu, score_cpu, kcache_cpu, vcache_cpu, base::DeviceType::kDeviceCPU,
  //     nullptr);
  ...
  //     start_pos, head_num, layer_index, seq_len, kv_dim, kv_mul, head_size, seq_len, start_pos +
  //     T, out_cu, q_cu, score_cu, kcache_cu, vcache_cu, base::DeviceType::kDeviceCUDA, &config);

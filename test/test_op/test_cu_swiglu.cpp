...
TEST(test_swiglu_cu, swiglu_batch_cpu_vs_cuda) {
  auto alloc_cu = base::CUDADeviceAllocatorFactory::get_instance();
  auto alloc_cpu = base::CPUDeviceAllocatorFactory::get_instance();
  ...
  tensor::Tensor in_cpu(base::DataType::kDataTypeFp32, B, H, true, alloc_cpu);
  tensor::Tensor wei_cpu(base::DataType::kDataTypeFp32, B, H, true, alloc_cpu);
  tensor::Tensor out_cpu(base::DataType::kDataTypeFp32, B, H, true, alloc_cpu);

  for (int i = 0; i < size; ++i) {
    in_cpu.index<float>(i) = 0.01f * float(i % 97 - 48);
    wei_cpu.index<float>(i) = 0.02f * float(i % 89 - 44);
  }
  ...
  for (int i = 0; i < size; ++i) {
    ASSERT_NEAR(out_cu.index<float>(i), out_cpu.index<float>(i), 1e-5f) << "i=" << i;
  }
}
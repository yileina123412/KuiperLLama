...
static void build_q8_and_sq4_from_fp32(const tensor::Tensor& w_fp32, int K, int M, int group_size,
...
TEST(test_matmul_cu, matmul_batch_cpu_vs_cuda) {
  auto alloc_cu = base::CUDADeviceAllocatorFactory::get_instance();
  auto alloc_cpu = base::CPUDeviceAllocatorFactory::get_instance();
  ...
  // weight: [K, M]
  const int K = 4;
  tensor::Tensor weight_cpu(base::DataType::kDataTypeFp32, K, M, true, alloc_cpu);

  // fill deterministic values
  for (int i = 0; i < B * M; ++i) input_cpu.index<float>(i) = float(i + 1);          // 1..6
  for (int i = 0; i < K * M; ++i) weight_cpu.index<float>(i) = float(i + 1) * 0.1f;  // 0.1..1.2
  ...
  // outputs
  tensor::Tensor out_cpu(base::DataType::kDataTypeFp32, B, K, true, alloc_cpu);
  tensor::Tensor out_cu(base::DataType::kDataTypeFp32, B, K, true, alloc_cu);
  ...
  kernel::get_matmul_kernel(base::DeviceType::kDeviceCPU)(input_cpu, weight_cpu, out_cpu, 1.f,
                                                          nullptr);
  kernel::get_matmul_kernel(base::DeviceType::kDeviceCUDA)(input_cu, weight_cu, out_cu, 1.f,
                                                           &config);
  ...
  for (int i = 0; i < out_cpu.size(); ++i) {
    ASSERT_NEAR(out_cu.index<float>(i), out_cpu.index<float>(i), 1e-4);
  }

  // cudaStreamDestroy(stream);
}

TEST(test_matmul_cu, bench_q8_vs_sq4_cuda_ncu) {
  auto alloc_cu = base::CUDADeviceAllocatorFactory::get_instance();
  auto alloc_cpu = base::CPUDeviceAllocatorFactory::get_instance();

  // 你可按显存调大：M/K越大越适合Ncu观察
  const int B = 16;            // batch
  const int M = 4096;          // input dim
  const int K = 4096;          // output dim
  ...
  tensor::Tensor input_cu = input_cpu.clone();
  input_cu.to_cuda(nullptr);
  tensor::Tensor w_fp32_cu = w_fp32_cpu.clone();
  w_fp32_cu.to_cuda(nullptr);
  ...
  tensor::Tensor out_fp32_cu(base::DataType::kDataTypeFp32, B, K, true, alloc_cu);
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    kernel::get_matmul_kernel(base::DeviceType::kDeviceCUDA)(input_cu, w_fp32_cu, out_fp32_cu, 1.f,
                                                             &cfg);
  }
  ...
  ASSERT_EQ(cudaEventSynchronize(ed), cudaSuccess);
  float fp32_ms = 0.f;
  ASSERT_EQ(cudaEventElapsedTime(&fp32_ms, st, ed), cudaSuccess);

  // 防止被当成“无副作用”路径，顺便看数值是否正常
  out_q8_cu.to_cpu();
  out_sq4_cu.to_cpu();
  out_fp32_cu.to_cpu();
  double sum_q8 = 0.0, sum_sq4 = 0.0, sum_fp32 = 0.0;
  ...
  for (int i = 0; i < out_fp32_cu.size(); ++i) sum_fp32 += out_fp32_cu.index<float>(i);
  ...
  LOG(INFO) << "[NCU_BENCH] checksum fp32=" << sum_fp32 << ", q8=" << sum_q8 << ", sq4=" << sum_sq4;

  ASSERT_TRUE(std::isfinite(sum_q8));
  ASSERT_TRUE(std::isfinite(sum_sq4));
  ASSERT_TRUE(std::isfinite(sum_fp32));

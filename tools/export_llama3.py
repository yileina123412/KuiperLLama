...
def load_hf_model(model_path):
    ...
    # HuggingFace permutes WQ/WK for RoPE; llama2.c-style weights expect the reverse permutation.
    # Reference: transformers convert_llama_weights_to_hf.py
    ...
    # In GQA, k_proj rows = kv_dim = head_size * n_kv_heads
    n_kv_heads = config.n_heads if config.n_kv_heads is None else config.n_kv_heads
    kv_dim = config.dim * n_kv_heads // config.n_heads

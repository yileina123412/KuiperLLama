...
__global__ void multi_head_attention_kernel(int32_t pos, int32_t seq_len, float* query,
                                            ...
                                            int32_t kv_prefix_keep_tokens, int32_t layer_offset) {
  int head = blockIdx.x;
  ...
  for (int i = threadIdx.x; i < head_size; i += blockDim.x) s_query_head[i] = query_head[i];
  __syncthreads();
  ...
  const int tail_cap = window - prefix;

  const int seen = pos + 1;
  const int valid_req = max(1, min(kv_valid_len, window));
  const int valid = min(valid_req, seen);

  const int prefix_visible = min(prefix, valid);
  const int tail_visible = max(0, valid - prefix_visible);
  const int attn_len = max(1, prefix_visible + tail_visible);
  const int tail_start = pos - tail_visible + 1;

  float* score_head = score_ptr + head * seq_len;
  int head_offset = (head / kv_mul) * head_size;

  for (int idx = threadIdx.x; idx < attn_len; idx += blockDim.x) {
    int logical_t = (idx < prefix_visible) ? idx : (tail_start + (idx - prefix_visible));
    int slot = (logical_t < prefix) ? logical_t : (prefix + ((logical_t - prefix) % tail_cap));
    ...
    score *= scale;
    score_head[idx] = score;
  }
  __syncthreads();

  softmax_gpu(score_head, attn_len);
  __syncthreads();

  float* output_head = output + head * head_size;
  for (int i = threadIdx.x; i < head_size; i += blockDim.x) {
    float value = 0.0f;
    for (int idx = 0; idx < attn_len; ++idx) {
      int logical_t = (idx < prefix_visible) ? idx : (tail_start + (idx - prefix_visible));
      int slot = (logical_t < prefix) ? logical_t : (prefix + ((logical_t - prefix) % tail_cap));
...
__global__ void multi_head_attention_prefill_kernel(
  ...
  const int prefix_visible = min(prefix, seen);
  const int tail_visible = min(max(seen - prefix, 0), tail_cap);
  const int attn_len_t = max(1, prefix_visible + tail_visible);
  const int tail_start = pos_t - tail_visible + 1;
  ...
  for (int idx = threadIdx.x; idx < attn_len_t; idx += blockDim.x) {
    int logical_p = (idx < prefix_visible) ? idx : (tail_start + (idx - prefix_visible));
    int slot = (logical_p < prefix) ? logical_p : (prefix + ((logical_p - prefix) % tail_cap));
  ...
  for (int idx0 = 0; idx0 < attn_len_t; idx0 += blockDim.x) {
    ...
    const int tile_len = min((int)blockDim.x, attn_len_t - idx0);
    for (int i = threadIdx.x; i < head_size; i += blockDim.x) {
      float acc = 0.f;
      for (int j = 0; j < tile_len; ++j) {
        int id = idx0 + j;
        int logical_p = (id < prefix_visible) ? id : (tail_start + (id - prefix_visible));
        int slot = (logical_p < prefix) ? logical_p : (prefix + ((logical_p - prefix) % tail_cap));
...
void mha_kernel_cu(int32_t pos, int32_t head_num, int32_t layer_index, int32_t seq_len,
  ...
  if (query_tensor.dims_size() == 1) {
    ...
    multi_head_attention_kernel<<<head_num, thread_num, head_size * sizeof(float), stream>>>(
        ...
        head_size, kv_window_size, kv_valid_len, kv_prefix_keep_tokens, layer_offset);
    return;
  }
